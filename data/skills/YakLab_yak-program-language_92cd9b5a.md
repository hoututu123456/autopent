# 反连管理 | Yak Program Language

- 来源: https://www.yaklang.com/products/back-link/
...
- 这个时候， abc.example.com 将会被解析，会查询 abc.example.com 的 NS 记录和 A 记录
- 如果 NS 记录存在(ns1.example.com)，会向 ns1.example.com 查询 abc.example.com 的 A 记录。
- 如果 NS 不存在，则直接向公共服务器启用 A 记录查询。
...
- TLS 协议要先进行 TLS 握手（handshake） 操作。在 TLS 握手时，我们将会识别到这是一个 TLS 握手操作，客户端把服务器当作一个 TLS 服务来判断，那么服务端将会自动返回 TLS 应该收到的东西。
- RMI 的协议头为 JRMI，当我们收到客户端发来这几个字节时，我们就应该回复 RMI 应该识别的内容
- 当然 HTTP 更简单了，对吧？
