# 插件商店介绍和使用 | Yak Program Language

- 来源: https://www.yaklang.com/products/Plugin-repository/Plugin-repository/
...
- 一键下载 ：可将插件商店所有的插件一键下载到本地
- 勾选 ：可勾选想要的插件后点击下载进行使用
- 点赞 ：登录后可对喜欢的插件“点赞”
- 下载 ：点击可下载单个插件
- 小课堂 ：点击会自动打开浏览器跳转到B站，查看牛牛精心录制的视频课程
- 实时资讯 ：点击关注牛牛公众号，每周都会发布技术文章
- 官方文档 ：点击会自动打开浏览器跳转到官网，查看YAK语言和Yakit教程
- 搜索 ：可通过关键字和作者对插件进行搜索

### 插件导入 #

- 导入资源 ：点击导入资源-导入插件，即可导入外部或本地插件
- 第三方仓库源 ：支持从线上下载Nuclei插件
- 本地仓库 ：支持将导出的Yakit插件进行导入，方便无网环境进行使用
- 本地Yaml-poc ：支持导入本地的Nuclei插件
- 使用ID ：通过他人分享的插件ID导入插件
...
- 保存 ：填写完成以后点击保存，即可将插件保存在本地使用
- 同步至云端 ：将插件同步到云端，为方便管理，需登录账号以后上传插件，可选择私密或公开，公开插件需要审核后方能在插件商店中可见。
- 调试插件 ：调试前会将插件保存在本地再进行调试
...
- 私密 ：上传为私密插件只有自己能看见，所以无需经过自动检测与审核。私密插件也可以分享
- 公开 ：公开插件需展示在插件商店中，为保证无害和可用性，需先进行自动检测，通过自动检测后才会上传；上传成功审核通过，才会在插件商店中可见。
...
- 复制到云端：如修改他人的插件不想经过作者合并再合并到云端，可以选择复制一份上传，但复制的插件只能为私密插件，不能改为公开。
